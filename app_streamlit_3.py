...
# ===== 常量（同 v5） =====
_SALES_SHEETS    = ["销量-5001","销量-5002"]
_TRANSFER_SHEETS = ["销量-转调理品原料","销量-调拨宫产量"]
...
def read_bom_mapping(uploaded):
    ...
    _force_put(["ABB0700210118","ABB0700491610","ABB0700493630","ABB0700493638","ADB0700493600"], "胸类-胸皮")
...
try:
    ...
    if overview is None or (hasattr(overview, "empty") and overview.empty):
        ...
        # 2) 勾选“用于相乘的部位产成率”（多选）
        common_targets = ["腿类","胸类","里肌类","翅类","骨架类","爪类","鸡肝类","鸡心类","鸡头类","脖类","鸡胗类","油类","下料类","其他内脏","整鸡类"]
        ...
        if not (pd.notna(src_qty) and float(src_qty) > 0):
            st.warning(f"⚠️ 还原部位【{restore_part}】在该日没有可用的产量。")
            st.stop()
        ...
        total_restore = float(src_qty * sum_rate)  # 还原产量（求和）
        normalized_restore = (total_restore / sum_rate) if sum_rate > 0 else 0.0  # 还原产量（求和 ÷ 产成率合计）
        ...
        # === 分布后总览：按“归一化还原量”进行分配，确保增量之和 = 归一化还原量 ===
        if sum_rate > 0:
        ...
        for p in show_parts:
            ...
            if p == "整鸡类":
                # 整鸡：只保留原产量；实际产量/产成率为 0；还原产量= -原产量
                new_qty = 0.0
                inc_show = -orig_qty  # ✅ 还原产量 = 产量加负号
                rate_after = 0.0
            else:
                new_qty = orig_qty + inc_qty  # 其他部位保持原逻辑
                inc_show = inc_qty
            ...
            rows.append({
                ...
                "增量(kg)": round(inc_show,2),  # （显示名：还原产量）
                "分布后产量(kg)": round(new_qty, 2),  # （显示名：实际产量）
        ...
        # —— 总计：不含【整鸡类 / 鸡头+鸡脖+骨架 / 胸类-胸 / 胸类-胸皮】；并计算“分布后总产成率(%)” ——         # —— 总计口径（分布后总览）：排除整鸡类、鸡头+鸡脖+骨架；并在胸类与两个子类间互斥 ——
        excluded = {'整鸡类','鸡头+鸡脖+骨架','胸类'}
        ov_included = ov_show[~ov_show['项目'].isin(excluded)].copy()
        # 产量总计 = 不含集合的原产量 + 整鸡原产量（你之前已这样做的话保持不变）
        ...
        # ✅ 还原产量（增量）总计：对整个显示表 ov_show 求和，整鸡的负值也参与
        ...
        def _fmt_price(v):
            try:
                ...
                if isinstance(v, str):
                    ...
                    if s == "" or s.lower() == "nan":
                        return ""
                    v = float(s)  # 支持把 "12.3" 这类字符串转成数值
                return f"{float(v):.2f}"
            except Exception:
                return ""  # 任何异常都返回空，避免抛错
