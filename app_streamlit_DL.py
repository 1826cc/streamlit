...
trans_upload = st.sidebar.file_uploader("转调理品原料", type=["xlsx","xls"])
transfer_upload = st.sidebar.file_uploader("转调拨/调拨宫产量", type=["xlsx","xls"])
lw_upload    = st.sidebar.file_uploader("净重（可选上传）", type=["xlsx","xls","csv"])
bom_upload   = st.sidebar.file_uploader("物料清单（可选上传）", type=["xlsx","xls","csv"])
...
required_main = [("产量", prod_upload), ("销量", sale_upload), ("转调理品原料", trans_upload)]
...
if load_errors:
    for msg in load_errors:
        st.warning(msg)
if xls is None:
    st.error("请上传完整的主数据文件（产量/销量/转调理品原料）。")
    st.stop()
if df_lw is None or df_lw.empty:
    st.warning("未上传或未读取到净重文件，将无法显示产成率。")
if not code2major:
    st.warning("未上传或未读取到物料清单，部分部位映射可能缺失。")
